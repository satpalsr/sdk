<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [server](./server.md) &gt; [HYTOPIA](./server.hytopia.md) &gt; [PlayerCamera](./server.hytopia.playercamera.md) &gt; [setModelHiddenNodes](./server.hytopia.playercamera.setmodelhiddennodes.md)

## HYTOPIA.PlayerCamera.setModelHiddenNodes() method

Sets the nodes of the model the camera is attached to that will not be rendered for the player. Uses case insensitive substring matching.

**Signature:**

```typescript
setModelHiddenNodes(modelHiddenNodes: string[]): void;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

modelHiddenNodes


</td><td>

string\[\]


</td><td>

Determines nodes to hide that match these case insensitive substrings.


</td></tr>
</tbody></table>
**Returns:**

void

