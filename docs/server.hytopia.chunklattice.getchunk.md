<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [server](./server.md) &gt; [HYTOPIA](./server.hytopia.md) &gt; [ChunkLattice](./server.hytopia.chunklattice.md) &gt; [getChunk](./server.hytopia.chunklattice.getchunk.md)

## HYTOPIA.ChunkLattice.getChunk() method

Get a chunk by its origin coordinate.

**Signature:**

```typescript
getChunk(originCoordinate: Vector3Like): Chunk | undefined;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

originCoordinate


</td><td>

[Vector3Like](./server.vector3like.md)


</td><td>

The origin coordinate of the chunk to get.


</td></tr>
</tbody></table>
**Returns:**

[Chunk](./server.chunk.md) \| undefined

The chunk at the given origin coordinate or undefined if not found.

