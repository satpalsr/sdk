<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [server](./server.md) &gt; [HYTOPIA](./server.hytopia.md) &gt; [Collider](./server.hytopia.collider.md) &gt; [setFrictionCombineRule](./server.hytopia.collider.setfrictioncombinerule.md)

## HYTOPIA.Collider.setFrictionCombineRule() method

Sets the friction combine rule of the collider.

**Signature:**

```typescript
setFrictionCombineRule(frictionCombineRule: CoefficientCombineRule): void;
```

## Parameters

<table><thead><tr><th>

Parameter


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

frictionCombineRule


</td><td>

[CoefficientCombineRule](./server.coefficientcombinerule.md)


</td><td>

The friction combine rule of the collider.


</td></tr>
</tbody></table>
**Returns:**

void

