<!-- Do not edit this file. It is automatically generated by API Documenter. -->

[Home](./index.md) &gt; [server](./server.md) &gt; [HYTOPIA](./server.hytopia.md) &gt; [Player](./server.hytopia.player.md)

## HYTOPIA.Player class

A player in the game.

**Signature:**

```typescript
export default class Player 
```

## Remarks

Players are automatically created when they connect and authenticate with the game server. This is all handled internally. Player instances should never be instantiated directly.

The constructor for this class is marked as internal. Third-party code should not call the constructor directly or create subclasses that extend the `Player` class.

## Properties

<table><thead><tr><th>

Property


</th><th>

Modifiers


</th><th>

Type


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

[id](./server.hytopia.player.id.md)


</td><td>

`readonly`


</td><td>

number


</td><td>

The unique identifier for the player.


</td></tr>
<tr><td>

[inputState](./server.hytopia.player.inputstate.md)


</td><td>

`readonly`


</td><td>

Readonly&lt;[PlayerInputState](./server.playerinputstate.md)<!-- -->&gt;


</td><td>

The current [PlayerInputState](./server.playerinputstate.md) of the player.


</td></tr>
<tr><td>

[orientationState](./server.hytopia.player.orientationstate.md)


</td><td>

`readonly`


</td><td>

Readonly&lt;[PlayerOrientationState](./server.playerorientationstate.md)<!-- -->&gt;


</td><td>

The current [PlayerOrientationState](./server.playerorientationstate.md) of the player.


</td></tr>
<tr><td>

[username](./server.hytopia.player.username.md)


</td><td>

`readonly`


</td><td>

string


</td><td>

The username for the player.


</td></tr>
<tr><td>

[world](./server.hytopia.player.world.md)


</td><td>

`readonly`


</td><td>

[World](./server.world.md) \| undefined


</td><td>

The current [World](./server.world.md) the player is in.


</td></tr>
</tbody></table>

## Methods

<table><thead><tr><th>

Method


</th><th>

Modifiers


</th><th>

Description


</th></tr></thead>
<tbody><tr><td>

[disconnect()](./server.hytopia.player.disconnect.md)


</td><td>


</td><td>

Disconnects the player from the game server.


</td></tr>
<tr><td>

[joinWorld(world)](./server.hytopia.player.joinworld.md)


</td><td>


</td><td>

Joins a player to a world.


</td></tr>
<tr><td>

[leaveWorld()](./server.hytopia.player.leaveworld.md)


</td><td>


</td><td>

Removes the player from the current [World](./server.world.md) they are in.


</td></tr>
</tbody></table>
